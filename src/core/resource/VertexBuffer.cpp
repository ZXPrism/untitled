...
namespace core {
    ...
    VertexBuffer &VertexBuffer::UpdateData(unsigned offsetFloats, unsigned newDataSizeFloats, float *newData)
    {
        if (offsetFloats + newDataSizeFloats >= _VBOSize)
        {
            std::cout << "[GLReady / VertexBuffer::UpdateData] fatal: Too much new data!" << std::endl;
            return *this;
        }

        glBindBuffer(GL_ARRAY_BUFFER, _VBO);
        glBufferSubData(GL_ARRAY_BUFFER, sizeof(float) * offsetFloats, sizeof(float) * newDataSizeFloats,
                        newData);
    ...
    VertexBuffer &VertexBuffer::EndSetUsage()
    {
        ...
        {
            glVertexAttribPointer(location, nFloats, GL_FLOAT, GL_FALSE, _totalOffset * sizeof(float),
